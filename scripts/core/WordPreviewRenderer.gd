...
func generate_preview(difficulty_name: String):
	...
	if not branch_preview_scene or not is_instance_valid(tile_container) or not (DifficultyLayouts is Node) \
	   ...
	   or preview_grid_width_in_tiles <= 0 or preview_grid_height_in_tiles <= 0: # Check fixed grid sizes too
		printerr("WordPreviewRenderer: Aborting generate_preview due to missing components or invalid settings.")
		# Do not change self.size here if it's meant to be fixed from _ready
		return
	...
	for tile_info in layout_data:
		if not ("tile_type_enum" in tile_info and "pos" in tile_info and "rot_idx" in tile_info):
			printerr("WordPreviewRenderer: Invalid tile_info for '", difficulty_name, "': ", tile_info)
			continue

		var tile_type_enum_value = tile_info.tile_type_enum
		var absolute_grid_pos: Vector2i = tile_info.pos # Use pos directly as absolute
		var rotation_idx: int = tile_info.rot_idx
		
		# Check if the absolute_grid_pos is within our defined preview canvas bounds
		if absolute_grid_pos.x < 0 or absolute_grid_pos.x >= preview_grid_width_in_tiles or \
		   absolute_grid_pos.y < 0 or absolute_grid_pos.y >= preview_grid_height_in_tiles:
			printerr("WordPreviewRenderer: Tile pos ", absolute_grid_pos, " for '", difficulty_name, 
					 "' is outside defined preview_grid bounds (", preview_grid_width_in_tiles, "x", preview_grid_height_in_tiles, "). Skipping tile.")
			continue
		...
		# Position the tile instance at the CENTER of its ABSOLUTE grid cell
		...
		tile_instance.set_tile_visuals(tile_type_enum_value, rotation_idx)
			
	# print("WordPreviewRenderer: Instantiated ", tile_instance_count, " tile previews for '", difficulty_name, "'.")
	# The SubViewport size is fixed, set in _ready(), so no need to resize it here per difficulty.
